...
import { computeRemaining, pickedKnockoutTeamIds, type RemainingPrediction } from './remaining';
...
describe('computeRemaining', () => {
  it('sums every alive pick when nothing is decided (optimistic ceiling)', () => {
    const preds: RemainingPrediction[] = [
      { userId: 'a', bonus: { finalist_1: 'BRA', finalist_2: 'ARG', bronze: 'FRA', champion: 'BRA' } },
    ];
    const [r] = computeRemaining({ predictions: preds, eliminatedTeamIds: [], decided: NONE });
    // 8 + 8 + 8 + 16
  ...
  it('drops a category when its team is eliminated', () => {
    const preds: RemainingPrediction[] = [
      { userId: 'a', bonus: { finalist_1: 'BRA', champion: 'BRA', bronze: 'FRA' } },
    ];
    const [r] = computeRemaining({ predictions: preds, eliminatedTeamIds: ['BRA'], decided: NONE });
  ...
  it('drops a category once its deciding match is played (avoids double count)', () => {
    const preds: RemainingPrediction[] = [{ userId: 'a', bonus: { champion: 'BRA', bronze: 'FRA' } }];
  ...
  it('both finalists share the finalists-decided flag', () => {
    const preds: RemainingPrediction[] = [{ userId: 'a', bonus: { finalist_1: 'BRA', finalist_2: 'ARG' } }];
    const [r] = computeRemaining({
      predictions: preds,
      eliminatedTeamIds: [],
    ...
    expect(r.categories.filter((c) => c.key.startsWith('finalist')).every((c) => c.decided)).toBe(true);
  });
  ...
  it('missing picks contribute nothing', () => {
    ...
    expect(r.reachable).toBe(0);
    expect(r.categories.every((c) => c.teamId === null && !c.counts)).toBe(true);
  });
});

describe('pickedKnockoutTeamIds', () => {
  it('returns the de-duplicated union of knockout picks', () => {
    const preds: RemainingPrediction[] = [
      { userId: 'a', bonus: { finalist_1: 'BRA', champion: 'BRA' } },
      { userId: 'b', bonus: { finalist_1: 'ARG', bronze: 'FRA', group_winner_A: 'ESP' } },
    ];
    expect(pickedKnockoutTeamIds(preds).sort()).toEqual(['ARG', 'BRA', 'FRA']);
  });
});
