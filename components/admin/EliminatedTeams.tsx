...
export function EliminatedTeams({ initial }: { initial: EliminationEntry[] }) {
  ...
  async function toggle(teamId: string, eliminated: boolean) {
    ...
    if (res.ok) setTeams((ts) => ts.map((t) => (t.teamId === teamId ? { ...t, eliminated } : t)));
    setBusy(null);
  }

  if (teams.length === 0) {
    return <p className="muted" style={{ fontSize: 12 }}>Inga slutspels-tips att visa än.</p>;
  }

  return (
    <div className="stack" style={{ gap: 6 }}>
      {teams.map((t) => (
        <div key={t.teamId} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
          <span style={{ fontWeight: 700, textDecoration: t.eliminated ? 'line-through' : 'none', opacity: t.eliminated ? 0.55 : 1 }}>
            {t.teamName}
          </span>
          <button
            type="button"
